...
def make_controller(prompts: List[str], is_replace_controller: bool,
                    ...
                    tokenizer,
                    blend_words=None, equalizer_params=None) -> AttentionControlEdit:
    ...
    if equalizer_params is not None:
        eq = get_equalizer(prompts[1], equalizer_params["words"], equalizer_params["values"], tokenizer=tokenizer)
...
def p2p_text2image(
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
        ...
        latents, _, noise_pred = ptp_utils.diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource=False, return_x0=return_pred)
        if return_pred:
    ...
    if return_pred:
        return image, latent, noise_pred_list
    return image, latent
